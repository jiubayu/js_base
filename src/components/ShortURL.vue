...
export default {
  ...
  methods: {
    createDB() {
      // hrome54并不会触发upgradeneeded事件，造成objectStoret不会被创建
      let request = window.indexedDB.open("shortURL", 2);
      request.onerror = (err) => {
        console.log(err);
      };
      request.onsuccess = (e) => {
        console.log('indexedDB.ope')
        this.db = e.target.result;
      };
      request.onupgradeneeded = (e) => {
        this.db = e.target.result;
        let objectStore = this.db && this.db.createObjectStore("shortURLs", {
          keyPath: "id",
          autoIncrement: true,
        });
    ...
    async doShort() {
      ...
      if (this.db) {
        ...
        objectStore.add({
          ...
          this.shortUrl = "http://a.b/" + this.shortUrl;
        };
      } else {
        console.log("error");
      }
    ...
    async update(obj) {
      let objectStore = this.getObjectStore();
      console.log(objectStore, objectStore.put, 'put-----')
